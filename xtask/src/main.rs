...
fn main() {
    ...
    let result = match arg_refs.as_slice() {
        ...
        ["verify-source-archive"] => verify_source_archive(),
        ["verify", "cards"] => {
            let root = repo_root();
        ...
        ["equation-batch", "verify", rest @ ..] => run_equation_batch_verify(rest),
        ["formula-registry", "generate", rest @ ..] => run_formula_registry_generate(rest),
...
fn run_equation_batch_verify(args: &[&str]) -> Result<(), String> {
    let root = repo_root();
    let options = equation_batch::verify::VerifyOptions::parse_args(args)?;
    equation_batch::verify::run_verify_command(&root, &options)
}

fn run_formula_registry_generate(args: &[&str]) -> Result<(), String> {
    let root = repo_root();
    let options = formula_registry::GenerateOptions::parse_args(args)?;
    formula_registry::run_generate_command(&root, &options)
}

fn run_formula_registry_generate_rust(args: &[&str]) -> Result<(), String> {
    let root = repo_root();
    let options = formula_registry::rust::GenerateRustOptions::parse_args(args)?;
...
fn verify_all() -> Result<(), String> {
    let root = repo_root();
    verify_source_archive_at_root(&root)?;
    release_identity::verify_release_identity(&root)?;
    Ok(())
}

fn verify_source_archive() -> Result<(), String> {
    let root = repo_root();
    verify_source_archive_at_root(&root)
}

fn verify_source_archive_at_root(root: &Path) -> Result<(), String> {
    verify_schema(root)?;

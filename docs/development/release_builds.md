...
The alpha Cargo package boundary contains `aero-codex-core`, `aero-codex-constants`, `aero-codex-astrodynamics`, and `aero-codex-cli`. CI creates package archives for all four with `cargo package --locked --no-verify`; registry publication remains a separate maintainer action because interdependent alpha packages are not yet present in a public registry. Every other workspace package, including `xtask`, is `publish = false`.

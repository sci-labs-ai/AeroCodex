...
- `cargo run -p xtask -- verify-release-manifest` parses only the v3 authority at `release/release-manifest.toml` and rejects the obsolete path, missing/unknown/duplicate/noncanonically ordered metadata, and any schema or channel drift. It requires exact workspace-order packages, the 12 dispatch-linked formulas, the 140-formula registry complement, four target triples with their fixed tier/CI/source-only posture, and nine declared-only artifact contracts. Target references, safe artifact basenames, formats, extensions, version/target filename templates, and approved formal-task IDs are strict. All 152 formulas must derive as `research_required`, blocked, and non-public. Every repository reference must name an existing contained regular file; absolute paths, traversal, final symlinks, and canonical symlink escapes fail explicitly. This containment check is filesystem-based and still works in a source archive. The separate pinned-base check requires `.git`, resolves the base to a commit, and proves it is an ancestor of `HEAD`.

...
| `python scripts/verify_governance.py --repo .` | exits successfully | Formula-vault runtime resolutions, A11-A13 external dispositions, and the configured xtask governance verifiers completed as a group. |
...
| External M07 rows with terminal dispositions | 112 | A11-A13 record 93 aliases, 13 excluded internal helpers, and 6 contract-blocked rows. |
...
- the A13 verifier reports the remaining 34 vector-helper rows with 26 aliases, 5 excluded internal helpers, 3 contract blocks, and an aggregate remaining backlog of 1,211;

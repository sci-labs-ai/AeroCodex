...
| `python scripts/verify_governance.py --repo .` | exits successfully | Formula-vault runtime resolutions, A11-A14 external dispositions, and the configured xtask governance verifiers completed as a group. |
...
| External M07 rows with terminal dispositions | 152 | A11-A14 record 109 aliases, 13 excluded internal helpers, and 30 contract-blocked rows. |
...
- the A14 verifier reports 40 classical two-body algebra rows with 16 aliases, 24 contract blocks, an unchanged medium-risk classifier label, 9 group rows deferred, and an aggregate remaining backlog of 1,171;

...
| `python scripts/verify_governance.py --repo .` | exits successfully | Formula-vault runtime resolutions, A11-A15 external dispositions, and the configured xtask governance verifiers completed as a group. |
...
| External M07 rows with terminal dispositions | 161 | A11-A15 record 115 aliases, 13 excluded internal helpers, and 33 contract-blocked rows. |
...
- the A14 verifier reports its first 40 classical two-body algebra rows and the A15 verifier reports the remaining 9 rows; together they cover all 49 rows with 22 aliases, 27 contract blocks, an unchanged medium-risk classifier label, and an aggregate remaining backlog of 1,162;

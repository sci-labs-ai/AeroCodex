...
Status: historical milestone documentation; the current release slice is `implementation_verified` and the non-release complement remains `research_required`.
...
The initial Beta 1 pilot directly exercised exactly ten canonical-unit kernels whose Rust runtime links, contracts, failure policies, numerical policies, family validation card, and family source seed were completed by M00-C2. The current CLI retains those ten dispatch links plus two angle-conversion links; the twelve audited rows are now `implementation_verified` / `normal_research` and publicly executable. This paragraph describes Beta 1 historically and the dispatch names as compatibility surfaces, not as the current release identity or a scientific validation claim.
...
## Historical Beta 1 acceptance gates
...
The public gate does not tag, publish, sign, package, upload, or certify a release. The current Cargo version is `0.1.0-alpha.1`; the release slice is `implementation_verified`; `beta1-concept` remains only a historical label and compatibility alias. See [`release_testing.md`](release_testing.md).

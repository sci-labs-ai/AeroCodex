...
## Initial pilot scope and current gate

The initial Beta 1 pilot directly exercised exactly ten canonical-unit kernels whose Rust runtime links, contracts, failure policies, numerical policies, family validation card, and family source seed were completed by M00-C2. The current CLI contains those ten dispatch links plus two angle-conversion dispatch links, but every corresponding registry row remains `research_required` and `blocked`; public `formula run` therefore fails closed before dispatch.
...
The ten canonical-unit formulas are the release-system self-check pilot, not the full equation program or a current public-execution claim. The repository contains a 1,000+ external equation backlog. The Beta 1 architecture proves the reusable path needed for later automated ingestion:

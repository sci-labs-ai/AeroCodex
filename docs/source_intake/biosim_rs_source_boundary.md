...
This source-intake note records the Stage 4 Chunk 4 boundary for BioSim-RS, the Chunk 6A clean-room resource/tick slice, the Chunk 6B clean-room atomic transaction commit slice, the Chunk 6C clean-room deterministic ordering/digest slice, the Chunk 6D clean-room resource-ledger slice, and the Chunk 6E clean-room CLI/API smoke slice. It does not import any source archive, does not promote GPL-bound implementation code, and does not change AeroCodex's current dual `MIT OR Apache-2.0` core.
...
## Allowed planning and Chunk 6A/6B/6C/6D/6E use
...
Chunk 6E adds only static clean-room API/example-output smoke reporting in `crates/aero-codex-life-support`, with source seed `source.life_support.biosim_rs.cli_api_smoke_clean_room.research_required` and validation card `life_support.biosim_rs.cli_api_smoke_friend_test_report`. It uses no external BioSim archive contents, no GPL-bound scaffold crates, no fixtures, no scenarios, and no persistent command surface, biological dynamics, habitat-control behavior, or external BioSim parity claim.
...
- adding persistent resource-ledger storage, broader conservation models, scenario-execution, persistent command surfaces, or habitat-control behavior without new slice-specific evidence;

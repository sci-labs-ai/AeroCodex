...
A13 subsequently processes the remaining 34 rows from `8D_helper_deduplication_then_low_risk_vector_contracts` using classifier metadata only. Twenty-six rows deduplicate to existing M00 vector runtimes, five internal column-shape helpers are excluded from formula scope, and three rows remain contract-blocked. A12-A13 now cover the full 74-row group. A14 then processes the first 40 rows of `8E_or_9A_classical_two_body_algebra_contracts`: 16 exact-name aliases reuse existing governed A7 runtimes and 24 rows remain contract-blocked. A14 preserves the classifier risk tier `medium_risk_requires_contract_review`; nine rows remain in that group. Current external accounting is 152 terminally processed rows and 1,171 remaining backlog rows. No raw Rust-port, M07, or Scilab source is imported or executed.

...
This README reflects current `main` after the bounded Stage 5 BioSim v3 B2c replay-integrity, deterministic ledger/report, synthetic example, and governance deployment. The live governed counters are verifier-derived; B2c intentionally adds one validation card, one source-registry seed, one validation-card-only inventory row, and twenty-four helper-algorithm rows while leaving executable research equations, metadata-only candidates, and external M07 backlog rows unchanged from B2b-2:

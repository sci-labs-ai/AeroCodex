...
This README reflects current `main` after the bounded Stage 5 final closeout/status consolidation on top of the deployed BioSim v3 B2c replay-integrity, deterministic ledger/report, synthetic example, and governance slice. The live governed counters are verifier-derived; B2c intentionally added one validation card, one source-registry seed, one validation-card-only inventory row, and twenty-four helper-algorithm rows while this final closeout changes no governed counts:

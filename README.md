...
This README reflects current `main` after compiler-batch coverage was completed for all 152 existing Rust equation runtimes, the 27 formula-vault intake records were linked to governed runtimes, and A11-A25 assigned terminal dispositions to fifteen bounded external-backlog waves. The live governed counters remain verifier-derived; these waves change no runtime kernel count and leave every linked formula at `research_required`:
...
| External M07 rows with terminal dispositions | 538 | A11-A25: 127 deduplicated aliases, 103 excluded internal/composite helpers, and 308 contract- or policy-blocked rows. |
| External M07 backlog rows | 785 | Registered external M07 represented rows that still lack a terminal disposition. |
...
A10 runtime-resolution status: `linked_to_existing_runtime=27`, `unresolved=0`, with 3 angle/unit, 14 vector-algebra, and 10 canonical-unit links in `formula-vault/resolutions/m00_runtime_links.tsv`. A11-A25 external status: `terminal_dispositions=538`, `deduplicated_aliases=127`, `excluded_internal_helpers=103`, and `contract_or_policy_blocked=308`, across fifteen bounded `formula-vault/resolutions/m07_*.tsv` manifests. A14-A15 complete the 49-row classical two-body algebra group. A16-A25 complete all 377 source-ordered rows of orbital-geometry and conic-branch review while preserving each classifier risk tier; none of these records claims M07/Scilab parity, certification, or operational readiness.

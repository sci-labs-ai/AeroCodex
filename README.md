...
This README reflects current `main` after the bounded Stage 5 BioSim v3 B2b-1 process-types, validated-constructor, and deterministic intent-planner deployment. The live governed counters are verifier-derived; B2b-1 intentionally adds twenty-eight helper-algorithm rows while leaving executable research equations, metadata-only candidates, external M07 backlog rows, validation cards, source-registry seeds, and validation-card-only records unchanged from B2a:
...
- clean-room BioSim-plus synthetic scenario-domain records, deterministic structural validation, process records, and one-tick intent-planning helpers for research metadata only.
...
| `aero-codex-life-support` | BLSS mass-balance helpers, thin-film/MELiSSA research kernels, clean-room BioSim-style resource/tick primitives, BioSim-plus synthetic scenario-domain structural validation, and bounded process/intent-planning helpers. |

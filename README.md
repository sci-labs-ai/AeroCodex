...
This README reflects current `main` after compiler-batch coverage was completed for all 152 existing Rust equation runtimes, the 27 formula-vault intake records were linked to governed runtimes, and A11-A15 assigned terminal dispositions to five bounded external-backlog waves. The live governed counters remain verifier-derived; these waves change no runtime kernel count and leave every linked formula at `research_required`:
...
| External M07 rows with terminal dispositions | 161 | A11-A15: 115 deduplicated aliases, 13 excluded internal helpers, and 33 contract-blocked rows. |
| External M07 backlog rows | 1,162 | Registered external M07 represented rows that still lack a terminal disposition. |
...
A10 runtime-resolution status: `linked_to_existing_runtime=27`, `unresolved=0`, with 3 angle/unit, 14 vector-algebra, and 10 canonical-unit links in `formula-vault/resolutions/m00_runtime_links.tsv`. A11-A15 external status: `terminal_dispositions=161`, `deduplicated_aliases=115`, `excluded_internal_helpers=13`, and `contract_blocked=33`, across five bounded `formula-vault/resolutions/m07_*.tsv` manifests. A14-A15 preserve the classifier risk tier `medium_risk_requires_contract_review` and complete the 49-row classical two-body algebra group; none of these records claims M07/Scilab parity, certification, or operational readiness.

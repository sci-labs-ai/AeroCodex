...
This README reflects current `main` after compiler-batch coverage was completed for all 152 existing Rust equation runtimes, the 27 formula-vault intake records were linked to governed runtimes, and A11-A18 assigned terminal dispositions to eight bounded external-backlog waves. The live governed counters remain verifier-derived; these waves change no runtime kernel count and leave every linked formula at `research_required`:
...
| External M07 rows with terminal dispositions | 281 | A11-A18: 121 deduplicated aliases, 45 excluded internal helpers, and 115 contract-blocked rows. |
...
A10 runtime-resolution status: `linked_to_existing_runtime=27`, `unresolved=0`, with 3 angle/unit, 14 vector-algebra, and 10 canonical-unit links in `formula-vault/resolutions/m00_runtime_links.tsv`. A11-A18 external status: `terminal_dispositions=281`, `deduplicated_aliases=121`, `excluded_internal_helpers=45`, and `contract_blocked=115`, across eight bounded `formula-vault/resolutions/m07_*.tsv` manifests. A14-A15 complete the 49-row classical two-body algebra group. A16-A18 process the first 120 source-ordered rows of orbital-geometry and conic-branch review while preserving each classifier risk tier; none of these records claims M07/Scilab parity, certification, or operational readiness.
